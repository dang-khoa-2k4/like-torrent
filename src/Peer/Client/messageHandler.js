...
module.exports.msgHandler = function (
  ...
  if (isHandshake(msg)) {
    ...
    if (m.id === 4)
      haveHandler(m.payload, socket, pieces, queue, peer, bitfield);
    if (m.id === 5) {
      bitfieldHandler(socket, pieces, queue, m.payload, peer, bitfield);
    }
    if (m.id === 7)
      pieceHandler(
        m.payload,
...
function pieceHandler(
  ...
  if (
    pieces._received[
      payload.index * torrentParser.blocksPerPiece(torrent, 0) + blockIndex
    ]
  ) {
  ...
  *Use for show download concurrently
  ...
  for (let i = 0; i < payload.block.length; i++) {
    piecesBuffer[
      payload.index * torrent.info["piece length"] + payload.begin + i
    ] = payload.block[i];
  }
  ...
  if (pieces.havePiece(payload.index)) {
    parentPort.postMessage({
      type: "have",
      payload: { index: payload.index, peer: peer },
    });
  }
...
function writeFilesFromPieces(fileInfoList, piecesBuffer, torrent) {
  ...
  fileInfoList.forEach((fileInfo) => {
    ...
    while (remainingLength > 0) {
      const pieceIndex =
        startPiece +
        Math.floor((offset + byteOffset) / torrent.info["piece length"]);
      const byteOffsetInPiece =
        (byteOffset + offset) % torrent.info["piece length"];
      ...
      const availableSpaceInPiece =
        torrent.info["piece length"] - byteOffsetInPiece;
      ...
      fs.writeSync(
        fileDescriptor,
        piecesBuffer.slice(
          pieceIndex * torrent.info["piece length"] + byteOffsetInPiece,
          pieceIndex * torrent.info["piece length"] +
            byteOffsetInPiece +
            bytesToWrite
        ),
        0,
