  ...
  return new Promise((resolve, reject) => {
    fs.readFile(fileInfo.path, (err, data) => {
      ...
      while (remainingLength > 0) {
        ...
        const pieceIndex =
          fileInfo.startPiece +
          Math.floor((offset + fileInfo.byteOffset) / PIECE_SIZE);
        ...
        piecesBuffer.set(
          data.slice(offset, offset + bytesToWrite),
          PIECE_SIZE * pieceIndex + byteOffsetInPiece
        );
        ...
        if (
          verifyPiece(
            piecesBuffer.slice(
              PIECE_SIZE * pieceIndex,
              Math.min(PIECE_SIZE * pieceIndex + PIECE_SIZE, tp.size(torrent))
            ),
            pieceIndex,
            torrent
          )
...
module.exports.processFiles = async function (fileInfoList, torrent) {
  ...
  const sharedRequestedBuffer = new SharedArrayBuffer(
    requestedSharedBufferSize
  );
  ...
  const readPromises = fileInfoList.map((fileInfo) =>
    readFileAndWritePieces(
      fileInfo,
      torrent,
      sharedPieceBufferView,
      pieces,
      pieceCount
    )
  );
