  ...
  needed(pieceBlock) {
    ...
    this.fileInfoList.forEach((file) => {
      ...
      const endPiece =
        startPiece +
        Math.floor(
          (file.byteOffset + file.length - 1) /
            this.torrent.info["piece length"]
        );
      ...
      for (let pieceIndex = startPiece; pieceIndex <= endPiece; pieceIndex++) {
        ...
        const offsetEnd = Math.min(
          offsetBegin + tp.blocksPerPiece(this.torrent, 0),
          this._numblocks
        );
        if (
          this._requested
            .slice(offsetBegin, offsetEnd)
            .every((block) => block == 1)
        ) {
  ...
  isDone(torrent) {
    ...
    for (const fileInfo of this.fileInfoList) {
      ...
      while (remainingLength > 0) {
        const pieceIndex =
          startPiece + Math.floor((offset + byteOffset) / PIECE_SIZE);
        ...
        const bytesInCurrentPiece = Math.min(
          remainingLength,
          PIECE_SIZE - ((offset + byteOffset) % PIECE_SIZE)
        );
