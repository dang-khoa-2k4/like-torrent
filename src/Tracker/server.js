  ...
  if (req.method === "GET") {
    ...
    if (action == "announce") {
      ...
      if (event === "started") {
        ...
        trackerDatabase.torrents[info_hash.data] = trackerDatabase.torrents[
          info_hash.data
        ...
        trackerDatabase.torrents[info_hash.data].forEach((peer) => {
          if (
            Buffer.compare(
              Buffer.from(peer.peer_id),
              Buffer.from(peer_id.data)
            ) == 0
          ) {
            // console.log(event);
            if (event === "completed" && peer.status !== "completed") {
              if (!downloadedForScrape[info_hash.data]) {
                downloadedForScrape[info_hash.data] = 0;
              }
      ...
      for (const torrent in trackerDatabase.torrents) {
        trackerDatabase.torrents[torrent] = trackerDatabase.torrents[
          torrent
        ].filter((peer) => Date.now() - peer.last_announce < timeOut);
        displayInfo(trackerDatabase, torrent);
      }

      const respone = {
        interval: 10000,
        complete: trackerDatabase.torrents[info_hash.data].filter(
          (peer) => peer.status === "completed"
        ).length, //number of completed
        incomplete: trackerDatabase.torrents[info_hash.data].filter(
          (peer) => peer.status !== "completed"
        ).length, //number of incomplete
        peers: getRandomPeers(
          trackerDatabase.torrents[info_hash.data].map((peer) => ({
...
function displayInfo(trackerDatabase, info_hash) {
  ...
  trackerDatabase.torrents[info_hash].forEach((torrent) => {
    ...
    table.push([
      ...
      downloaded,
      left,
      new Date(last_announce).toLocaleString(),
    ]);
  });
